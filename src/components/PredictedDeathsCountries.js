...
export class PredictedDeathsCountries extends Component {
    ...
    render() {
        return (
            ...
            <div style = {{background: "#bbb", borderRadius: "2%", padding: "10px"}}>
                <h2 style={{marginBottom:'10px', textAlign: "center", wordWrap: "normal", width:"260px"}}>Predicted Deaths by Countries</h2>
                        <div style= {{height:'250px', overflow:'auto', width: '260px'}}>
                            {this.state.sorted.map((data, i) => (
                                <div key={i} style={country }>
                                <h4 >{data.country}</h4>
                                <h4 style={{marginLeft:'15px'}}>{this.multiplyByTen(parseInt(data.deaths))}</h4>
                                </div>
                            ))}
                        </div>
                        </div>
...
export default PredictedDeathsCountries
